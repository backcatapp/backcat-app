/** Shared ask SSE client (mirrors web/lib/ask.ts) with optional Bearer auth. */

export const SERVE_URL = __SERVE_URL__;
...
export async function askStream(
  ...
  if (!resp.ok || !resp.body) {
    events.onError(`request failed (${resp.status})`, resp.status);
    return;
  }
  ...
  const dispatch = (block: string) => {
    ...
    if (event === "sources") events.onSources(payload as Source[]);
      ...
      const err = payload as { message: string; code?: number };
      events.onError(err.message, err.code);
      ended = true;
    } else if (event === "done") {
      events.onDone();
      ended = true;
